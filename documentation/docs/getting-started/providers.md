...
| [Gemini](https://ai.google.dev/gemini-api/docs)                             | Advanced LLMs by Google with multimodal capabilities (text, images). Gemini models support configurable [thinking levels](#gemini-thinking-levels).                                                                                                | `GOOGLE_API_KEY`, `GEMINI_THINKING_LEVEL` (optional)                                                                                                                              |
...
Gemini 2.0 and Gemini 3 models support configurable thinking levels to balance response latency and reasoning depth:
...
<Tabs groupId="interface">
  <TabItem value="ui" label="goose Desktop" default>
    When selecting a Gemini model that supports thinking, a "Thinking Level" dropdown appears automatically. Select your preference and the setting persists across sessions.
  </TabItem>
  
  <TabItem value="cli" label="goose CLI">
    **Interactive configuration:**
    
    When you run `goose configure` and select a Gemini model that supports thinking, you'll be prompted to choose a thinking level:
    
    ```
    ◆  Select thinking level for Gemini:
...
2. `GEMINI_THINKING_LEVEL` or `GEMINI3_THINKING_LEVEL` environment variable
3. Default value: `low`
:::
